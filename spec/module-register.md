...
最後更新：2026-06-11（M3.5 紅旗 1/2/3 全解除；M35-009 擱置待 LinkedIn API 審核）
...
|---|------|----------|---------|--------|--------------------------|------------|
| M1 | 帳號 / 訂閱 | dev login + JWT + plan/entitlement 開關 | §11.3、TECH_STACK §10 | 已實作（Free）+ Pro entitlements 進行中（未提交） | – | – |
...
| M3.5 | 個人 LinkedIn 補充 | Pro：LinkedIn 公開資料 + LLM → 職責摘要 | §11.2.1 C、§12.5（含 DDR-81~85）、DDR-74/75/76、SA-SD v1.1、ENG v1.0 | **防冒充就緒 — 待 QA + commit；M35-009 擱置** | [kickoff](kickoffs/m3_5-20260611.md)；pm＋sa-sd v1.1＋ENG v1.0＋M35-010 皆 2026-06-11 | ✅ 紅旗 1/2/3 全解除（防冒充降級保險 + 死碼清除 + 單元測試）；⛔ M35-009 接官方 LinkedIn API 擱置（API 待審核）；⏳ DB 整合測試待 Postgres 環境、變更未 commit |
| M5 | AI 搜尋（Pool A） | 對話式反向搜尋 + 匹配理由 + 硬條件 NO_MATCH | §5.2 P0、US-5.x、DDR-73 | 已完成 - 待驗證（committed，Pool A） | – | – |
| M6 | 公司補全 | 收錄時 enrich 主要產品 + provenance + 手動更新 + Pro stale refresh | §12、US-6.x、DDR-34~37 | 已實作（Free）+ Pro stale_refresh 進行中（未提交） | – | – |
...
| # | 模組 | 目的 | 對應 PRD | 階段 |
|---|------|------|---------|------|
| M11 | 企業電子名片（Publisher） | 建立公開員工電子名片 → Pool B | §13、DDR-60 | 未啟動（Phase 3） |
| M5b | Network 跨池搜尋 | Pool A + Pool B | §13.1、DDR-58 | 未啟動（Phase 3） |
