...
最後更新：2026-07-21（多輪搜尋＋對話紀錄 · DDR-v4-17 已確認）
PRD 現行：**`BSChat_PRD_v4.md`（已確認 2026-07-14；DDR-v4-13～17 於 2026-07-21 補）**；v3＝團隊情報層參考；wedge＝`spec/product/BSChat_network_wedge.md`（已拍板）
⚠️ 2026-07-14：**終局＝可發現網絡 + Plan 搜尋**；**PWA＝日常應用**；名片商級完善＝Later **後台**（非本 PWA）。詳見變更紀錄。

## 一句話定位

紙名片與企業公開身份都成為可被 AI 讀懂的資產；需求出現時，以可見的 Plan 搜你的庫，並在允許時推薦自願公開的身份（非聊天型社群）。**PWA 不是電子名片產生器**；名片商級能力若做，進後台系統且排 Later。
...
- **產品表面**：PWA＝日常（收錄／問 AI／輕量治理）；**後台系統（另建）**＝放大 Admin＋平台運營＋（Later）名片設計／名片商級完善
...
- 2026-07-21：**PWA vs 後台分層**（DDR-v4-12 補強／DDR-v4-14）— PWA 方便日常；後台放大 Admin 與平台運營；名片商基本功能完善但市場飽和 → **最後做、且只做在後台**，不塞進 PWA。
- 2026-07-21：**PWA 不做「可分享電子名片」**（DDR-v4-13）— 核心＝問 AI 搜庫＋企業公開推薦；外鏈＝官網／LinkedIn 等。分享連結／QR／`/card` 傳閱等待拆除。
- 2026-07-20：**企業公開身份建立預設允許 AI 推薦** — Admin 新增／CSV 預設 `allow_ai_recommend=true`；可取消改草稿。
- 2026-07-20：**子團隊 UI 與方案門檻** — 全員可建＋Admin 解散；「我的」總覽｜子團隊｜偏好；子團隊僅企業；平台後台列規劃（非 PWA）。
- 2026-07-20：**團隊池邊界與單張共享（需求訪談收斂）** — 共享＝僅所屬子團隊；單張共享不做；團隊共享 ≠「允許 AI 推薦」。
...
- 2026-07-20：**子團隊 UI 決策鎖定** — 全員可建＋主 Admin 可解散；「我的」三分頁；**子團隊僅企業**（DDR-v4-11）；SaaS Platform 管理後台列 Next（DDR-v4-12）。待 SA／ENG。
- 2026-07-20：**子團隊開工＝先 UI** — kickoff `subteam-pool-20260720` 通過；UIUX `BSChat_UIUX_subteam.md`；原型 `screen-8-subteam.html`。待用戶確認畫面後再 SA／ENG。單張共享不做。
- 2026-07-20：**團隊池＝子團隊；不做單張共享** — 用戶確認共享僅所屬子團隊、主管可建隊；單張名片共享僅作訪談紀錄、明確不做（PRD §2.5／§9、DDR-v4-10）。子團隊實作尚未開工（現況仍整公司一池）。
