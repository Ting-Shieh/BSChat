...
> OpenAI 路徑（v8）：`system`＝政策、`user`＝**僅當前這一句**（不把先前 turns 塞進 user，避免沿用 browse）。  
> 多輪歷史只用在檢索 `effective_query`，不用在意圖分類。  
> `browse_public` → 摘要＋**3** 張樣例＋追問建議；`browse_public_more` → 僅無過濾「列更多」。  
