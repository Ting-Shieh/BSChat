...
## 9. 增量 · Email 邀請＋F1 站內通知（2026-07-21）

> 對應拍板：A1／僅 owner／一 Email 一邀請／狀態列／F1＋Email；F2 Later。主 Admin **不發**子團隊邀請。

### 流程

1. Owner 在隊頁輸入已是本企業成員的 Email → `POST /sub-teams/{id}/invites` `{ email }`
2. 建 `team_invites`（`invite_kind=sub_team`、`invited_email`、`max_uses=1`）
3. 寫入 `user_notifications`（被邀者）＋寄 Email（失敗仍保留邀請＋回 `email_sent=false`）
4. Owner：`GET /sub-teams/{id}/invites` 看邀請中／已加入／已撤銷／已過期
5. 被邀者：站內通知接受，或 Email 連結 `/join/team/{token}`

### DB

`user_notifications`：`id, user_id, kind, title, body, payload jsonb, read_at, created_at`  
`kind=sub_team_invite`；payload 含 `invite_id, sub_team_id, sub_team_name, org_name`

### API 增量

| Method | Path | 說明 |
|--------|------|------|
| `POST` | `/sub-teams/{id}/invites` | body `{ email, expires_days? }`；**僅 owner**；A1 |
| `GET` | `/sub-teams/{id}/invites` | owner 看邀請列＋`status` |
| `POST` | `/sub-teams/invites/{invite_id}/revoke` | 僅該隊 owner |
| `GET` | `/notifications` | 我的站內通知 |
| `POST` | `/notifications/{id}/accept` | 接受子團隊邀請（email 須吻合） |
| `POST` | `/notifications/{id}/read` | 標已讀 |

錯誤：`NOT_OWNER`、`NOT_ORG_MEMBER`、`ALREADY_MEMBER`、`PENDING_EXISTS`、`EMAIL_MISMATCH`

### 不做（本輪）

Web Push（F2）、主 Admin 代邀、匿名多人共用連結（舊 API 行為退役）。
